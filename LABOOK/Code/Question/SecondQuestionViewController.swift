...
class SecondQuestionViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    
    let secondQuestionList = ["読んでほしい名前は？","生年月日は？","初めて会った時の印象は？","1番好きなところは？","1番好きな食べ物は？","嫌いな食べ物は？","インドア派？アウトドア派？","今まで1番良かったデートスポットは？","相手を一言で表すと？","1番仲良しな友達はどんな人？","好きな色は？","苦手な色は","子供の頃の将来の夢は？","会っていない時は何してることが多い？","今までしてくれたことで、1番嬉しかったことは？","甘えたい派？甘えられたい派？","これだけは許してほしい自分の短所は？","言いにくいけど、実は直してほしいことは？","たまにしてほしい服装は？","いつか二人で行きたいデートスポットは？","記念日やイベントは全力で楽しみたい派？","1年後の二人はどうなってると思う？","今までの人生で1番頑張ったことは？","来世は男の子がいい？それとも女の子？","感謝の一言をどうぞ！"]
    
    var mySecondQuestionAnswer = ["未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答","未回答"]
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return secondQuestionList.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        questionSubject.text =  "(" + String(describing: "\(indexPath.row + 1 )") + ")" + String(describing: secondQuestionList[indexPath.row])
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        vc.questionName = "secondQuestion"
        vc.transitionSource = "SecondQuestionViewController"
        let cell = QuestionTableView.dequeueReusableCell(withIdentifier: cellId, for: indexPath)
        vc.questionSubject = secondQuestionList[indexPath.row]
        let myAnswer = cell.contentView.viewWithTag(3) as! UITextView
    ...
    func fetchQandAData(){
        ...
        mySecondQuestionAnswer.removeAll()
        guard let email = Auth.auth().currentUser?.email else {return}
        secondQuestionList.forEach { question in
            Firestore.firestore().collection("question").document(email).collection("secondQuestion").document(question).getDocument {(snapshot, err ) in
                if let err = err{
                    ...
                    guard snapshot!.exists else{
                        ...
                        self.mySecondQuestionAnswer.append("未回答")
                        return
                    }
                    print("snapshot exists")
                    let dic = snapshot?.data()
                    let q = Question.init(dic: dic!)
                    self.mySecondQuestionAnswer.append(q.answer)
                    self.numberOfAnswered += 1
                }
                    
                }
            }
            answeredCountLabel.text = "\(numberOfAnswered)/\(secondQuestionList.count)問"
            if numberOfAnswered == 0 {
                answeredParcentProgressView.setProgress(0, animated: false)
            }else{
                answeredParcentProgressView.setProgress(Float(numberOfAnswered/secondQuestionList.count), animated: true)
            }
