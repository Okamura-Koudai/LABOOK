//
//  ForthSurveyViewController.swift
//  LABOOK
//
...
class ForthSurveyViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let email = Auth.auth().currentUser?.email{
            Firestore.firestore().collection("survey").document("forthSurvey").collection(surveyList[indexPath.row]).document(email).getDocument {(snapshot, err ) in
                if let err = err {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        VC.csvFileName = "ForthSurveyArray"
        VC.surveyName = "forthSurvey"
    ...
    override func viewDidLoad() {
        ...
        csvArray = loadCSV(fileName: "ForthSurveyArray")
    ...
    @IBAction func toThirdPageButton(_ sender: Any) {
        ...
        let pageViewController = self.parent as! SurveyPageViewController
        let nextIndex2 = 2
        pageViewController.setViewControllers([pageViewController.controllers[nextIndex2]], direction: .reverse, animated: true, completion: nil)
